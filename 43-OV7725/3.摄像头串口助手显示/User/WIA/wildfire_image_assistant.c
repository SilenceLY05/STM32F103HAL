...
/* 发送设置图像格式包给上位机 type:图像格式 width:图像宽度 height:图像高度 */
...
/* 读寄存器 addr:寄存器地址 status:0应答 -1非应答 */
...
void set_wincc_format(uint8_t addr,uint8_t type,uint16_t width,uint8_t height)
{
 ...
 packet_head[11] = width & 0xFF;                     //寄存器对应上位机的序号
 packet_head[12] = width >> 8;                       //寄存器对应上位机的序号

 packet_head[11] = height & 0xFF;                     //寄存器对应上位机的序号
 packet_head[12] = height >> 8;                       //寄存器对应上位机的序号
...
/* 发送图像数据包给上位机 addr:设备地址，0或1 width:图像宽度 height:图像高度 返回：0或者1 */
