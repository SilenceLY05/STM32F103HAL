#ifndef __BSP_OV7725_H__
#define __BSP_OV7725_H__
...
typedef struct 
{
	...
	uint16_t lcd_scan;           //液晶屏的扫描模式（0-7）
	
	uint16_t light_mode;         //光照模式，参数范围[0~5]
	int16_t saturation;          //饱和度,参数范围[-4 ~ +4]   
	int16_t brightness;          //光照度，参数范围[-4~+4]
	int16_t contrast;            //对比度，参数范围[-4~+4]
...
#define OV7725_VSYNC_EXTI_IRQHandler        EXTI3_IRQHandler
