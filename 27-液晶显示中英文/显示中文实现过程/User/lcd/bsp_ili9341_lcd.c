...
static void ILI9341_GPIO_Config(void)
{
	...
	GPIO_InitStructure.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_7 | GPIO_PIN_8 |
	                         GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_14 | GPIO_PIN_15;
	HAL_GPIO_Init(GPIOD,&GPIO_InitStructure);
	
	
	/* 初始化PE7,8,9,10,11,12,13,14,15 */
	GPIO_InitStructure.Pin = GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12 |
...
static void ILI9341_FSMC_Config(void)
{
	...
	SRAM_Handler.Init.MemoryType = FSMC_MEMORY_TYPE_NOR;                     //SRAM
...
void myshow_ch(uint16_t usx,uint16_t usy)
{
	uint8_t rowCount,bitCount;   //循环用到的行数和每行像素点个数(都是32)
	uint32_t usTemp;             //用于显示一行数据中间用到的变量
	uint8_t Buffer[32*32/8] = {0};
	
	ILI9341_OpenWindow(usx,usy,32,32);
	...
	{
		usTemp = Buffer[rowCount*4];
		usTemp = usTemp << 8;
		usTemp |= Buffer[rowCount*4 +1];
		usTemp = usTemp << 8;
		usTemp |= Buffer[rowCount*4 +2];
		usTemp = usTemp << 8;
		usTemp |= Buffer[rowCount*4 +3];
...
void myshow_chs(uint16_t usx,uint16_t usy,uint8_t *x)
{
	uint8_t rowCount,bitCount;   //循环用到的行数和每行像素点个数(都是32)
	uint32_t usTemp;             //用于显示一行数据中间用到的变量
	uint8_t Buffer[32*32/8] = {0};
	...
	//取字模数据
	HZ_num = sizeof(lcd_font1) / sizeof(LCD_Typedef_FONT);
	
	for(int k = 0;k<HZ_num;k++)
	{
		if((lcd_font1[k].Index[0] == *x) && (lcd_font1[k].Index[1] == *(x+1)))
		{
			for(int i=0; i<32*32/8;i++)
			{
				Buffer[i] = lcd_font1[k].Msk[i];
			}
		}
