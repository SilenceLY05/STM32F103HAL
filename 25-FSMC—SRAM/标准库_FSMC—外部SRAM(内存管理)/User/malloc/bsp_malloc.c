...
//定义一个大数组，数组所占的空间能被32整除，同时数据首地址指定为0x68000000这个地址
//__attribute__((align(n))),如果n大于此结构体中大小基本数据类型size，那么依据最大基本数据类型size对齐，否则，依据n进行对齐。
...
void *myreallco(uint8_t memx,void *ptr,uint32_t size)
{
	...
	{
		mymemcpy((void*)((uint32_t)mallco_dev.membase[memx]+offset),ptr,size);  //拷贝就内存内容到新内存
		myfree(memx,ptr);
